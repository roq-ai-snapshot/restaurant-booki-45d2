...
import { getIngredientInventories } from 'apiSdk/ingredient-inventories';
import { IngredientInventoryInterface } from 'interfaces/ingredient-inventory';
import { getMenuItems } from 'apiSdk/menu-items';
import { MenuItemInterface } from 'interfaces/menu-item';
import { getCustomerFeedbacks } from 'apiSdk/customer-feedbacks';
...
function DashboardPage() {
  ...
  const ingredientInventoryStat = useSWR<PaginatedInterface<IngredientInventoryInterface>>(
    () => `/ingredient-inventories`,
    () => getIngredientInventories({ limit: 1, offset: 0 }),
  );
  ...
  const customerFeedbackStat = useSWR<PaginatedInterface<CustomerFeedbackInterface>>(
    () => `/customer-feedbacks`,
    () => getCustomerFeedbacks({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Ingredient Inventories',
      result: ingredientInventoryStat?.data?.totalCount,
      isLoading: ingredientInventoryStat.isLoading,
    ...
    },
    {
      ...
      result: customerFeedbackStat?.data?.totalCount,
      isLoading: customerFeedbackStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
